...
const onSubmit = async () => {
    ...
    if (!haveName || !haveUrlImage || !haveCategory || !havePrice || !haveStock) {
        toastNotification({
    ...
    const product = {
        name: name.value, image: urlImage.value, category: category.value, price: price.value, stock: stock.value
    }
...
<template>
    <div class="q-pa-md sm:w-[450px]" style="max-width: 400px">
        ...
        <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-md">
            <q-input filled v-model="name" label="Nombre *" hint="Agregar nombre de Producto" lazy-rules
                :rules="[val => val && val.length > 0 || 'Por favor tipear un nombre']" />
            ...
            <q-input filled v-model="urlImage" label="Url Imagen *" hint="Agregar un Link de imagen para el Producto"
                lazy-rules :rules="[
                    ...
                    val => /^(https?:\/\/.*)$/.test(val) || 'Ingrese una URL válida de imagen'
                ]" />
            ...
            <q-input filled type="number" v-model="price" label="Precio *" lazy-rules :min="0.01" step="0.01" :rules="[
                ...
                val => parseFloat(val) > 0 || 'Por favor ingrese un valor numérico positivo'
            ]" />
            ...
            <q-input filled type="number" v-model="stock" label="Stock *" lazy-rules :min="1" :rules="[
                ...
                val => !isNaN(val) || 'Ingrese un número válido',
                val => Number(val) > 0 || 'Por favor ingrese un valor numérico positivo'
            ]" />
